        ...
        // v121: pdf.js — same vendored-locally pattern as border-day-ledger/tax-tracker/
        // wealth-planner (see CSP-hardening notes: no CDN dependency, no SRI question since
        // it's same-origin). Loaded once, lazily, the first time a PDF attachment is actually
        // opened — most sessions never touch a PDF at all, so there's no reason to pay the
        // ~500KB parse cost on every launch. As of pdfjs-dist 3.x/4.x+ this only ships an ES
        // module build, hence the dynamic import() from this plain (non-module) script rather
        // than a <script type="module">. The worker is vendored alongside it at
        // ./lib/pdf.worker.min.mjs — CSP's existing `worker-src 'self'` already covers it, no
        // CSP change needed for either file.
        ...
        // Holds the compressed base64 image, or a raw base64 PDF, (if any) currently attached
        // in the open transaction form. Which one it is gets sniffed later from the data: URL's
        // own mime prefix — no separate type field needed.
        let currentTxImageData = null;
        // Original filename of the currently-attached PDF (used for the viewer title + display
        // in the form). Always null for images — the compressed JPEG has no meaningful original
        // name and the receipt UI never showed one.
        let currentTxAttachmentName = null;
        ...
        async function openTransactionForm(type, existingTxId = null, presetSrcAccountId = null) {
            ...
            if (existingTxId !== null) {
                ...
                setTxImagePreview(tx.image || null, tx.attachmentName || null);
        ...
        // --- RECEIPT / PHOTO / PDF ATTACHMENT ---

        // v121: extended from image-only to also accept a PDF, following the same pattern
        // border-day-ledger used — sniff the mime prefix off the stored data: URL itself
        // rather than adding a separate "type" field, so existing records (`t.image` always an
        // image data URL) keep working unchanged; a PDF just happens to also be storable in
        // that same field now. `nameOrNull` is the original filename, only ever set for PDFs
        // (kept for the viewer title + this preview chip — a compressed-JPEG receipt never had
        // a meaningful original name to show).
        function setTxImagePreview(base64OrNull, nameOrNull) {
            currentTxImageData = base64OrNull;
            currentTxAttachmentName = base64OrNull ? (nameOrNull || null) : null;
            const wrap = document.getElementById("txImagePreviewWrap");
            const img = document.getElementById("txImagePreview");
            const pdfChip = document.getElementById("txPdfPreviewChip");
            const pdfName = document.getElementById("txPdfPreviewName");
            const status = document.getElementById("txImageStatus");
            const isPdf = !!base64OrNull && base64OrNull.startsWith("data:application/pdf");
            ...
            if (base64OrNull) {
                ...
                if (isPdf) {
                    ...
                    img.src = "";
                    pdfChip.style.display = "flex";
                    pdfName.textContent = currentTxAttachmentName || "Attachment.pdf";
                } else {
        ...
        function handleTxImageSelected(event) {
            ...
            // PDFs are stored as-is (no compression pipeline makes sense for a PDF) — just the
            // raw data: URL plus the original filename.
            if (isPdf) {
                status.textContent = "Reading PDF...";
                reader.onload = (e) => {
                    setTxImagePreview(e.target.result, file.name);
                    const approxKb = Math.round((e.target.result.length * 0.75) / 1024);
            ...
            reader.onload = (e) => {
                ...
                compressImage(rawDataUrl, 1024, 0.7)
                    .then((compressedDataUrl) => {
                        setTxImagePreview(compressedDataUrl, null);
        ...
        // v121: extended to also view PDF attachments, same mime-sniff-off-the-data-URL
        // approach as border-day-ledger. Deliberately NOT an <iframe src="blob:..."> / native
        // PDF plugin (inconsistent across platforms — Chrome/Android in particular can render
        // some PDFs blank via that path). pdf.js decodes the bytes in JS and renders each page
        // onto its own <canvas> instead, so the result is identical on every device and never
        // executes anything embedded in the PDF (isEvalSupported: false below is belt-and-
        // suspenders on top of only ever calling getPage()/render(), never touching any
        // AcroForm/embedded-JS path).
        async function openImageViewer(base64, event, name) {
            if (event) event.stopPropagation();
            if (!base64) return;
            const isPdf = base64.startsWith("data:application/pdf");
            const imgEl = document.getElementById("imageViewerImg");
            const pdfBody = document.getElementById("pdfViewerBody");
            const titleEl = document.getElementById("imageViewerTitle");
            titleEl.textContent = name || (isPdf ? "Attachment" : "");
            titleEl.style.display = titleEl.textContent ? "block" : "none";

            if (!isPdf) {
                pdfBody.style.display = "none";
                pdfBody.innerHTML = "";
            ...
            imgEl.src = "";
            pdfBody.style.display = "block";
            pdfBody.innerHTML = '<div style="color:#f8fafc; text-align:center; padding:30px;">Loading PDF…</div>';
            openModal("imageViewerModal");
            try {
                ...
                pdfBody.innerHTML = '<div style="color:#f8fafc; text-align:center; padding:30px;">Could not preview this PDF' + (err && err.message ? (": " + escapeHtml(err.message)) : "") + '.</div>';
            }
        }
        ...
        async function handleTransactionSubmitMobile() {
            ...
            const record = {
                ...
                attachmentName: currentTxAttachmentName || null,
            ...
            try {
                if (splitRows.length > 0) {
                    ...
                    for (const row of splitRows) {
                        ...
                        // Only the first (main) row in a split carries the receipt/attachment, if
                        // any — attaching the same one to every split part would be misleading.
                        extraRecord.image = null;
                        extraRecord.attachmentName = null;
        ...
        async function renderApp() {
            ...
            txs.sort((a,b) => new Date(b.date) - new Date(a.date)).forEach(t => {
                ...
                const receiptBadge = t.image
                    ? `<span data-click="openImageViewer" data-image="${escapeHtml(t.image)}" data-name="${escapeHtml(t.attachmentName || "")}" style="cursor:pointer; margin-left:4px;" title="${t.attachmentName ? "View attached PDF" : "View attached photo"}">📎</span>`
                    : '';
                const referenceText = t.fdReferenceNo ? ` · Ref: ${escapeHtml(t.fdReferenceNo)}` : '';
        ...
        const CLICK_ACTIONS = {
            ...
            openImageViewer: (el, e) => openImageViewer(el.dataset.image, e, el.dataset.name),
