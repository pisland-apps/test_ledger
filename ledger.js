        ...
        // ============================================================================
        // v264: Language / i18n (English, Simplified Chinese, Traditional Chinese)
        // ----------------------------------------------------------------------------
        // Design: dictionaries are keyed by the EXACT (trimmed) English source string as
        // it appears in index.html or in a t("...") call below — not by an invented key
        // namespace. That means adding a translation for a new string never requires
        // touching two places to keep a key in sync: t("Some New Label") either finds an
        // entry or silently falls back to the English text unchanged, so a missing
        // translation degrades to English rather than breaking anything.
        //
        // Static HTML (index.html) is marked up with data-i18n / data-i18n-placeholder /
        // data-i18n-title / data-i18n-aria-label / data-i18n-datatitle attributes —
        // applyI18nStatic() walks those on boot (before the lock screen even shows, so
        // "Enter Passcode" etc. is translated too) and again after switching language.
        // The *original* English text/attribute value is cached on first pass (WeakMaps
        // below) so re-applying after a language switch — or switching back to English —
        // is always correct regardless of how many times it runs.
        //
        // Dynamic content ledger.js builds itself (page titles, empty-state text, confirm
        // dialogs, etc.) is translated by wrapping the literal string in t("...") at the
        // point it's built — see getActivePageTitle() below for the pattern. This build
        // covers the full static app shell (nav/sidebar/forms/buttons/settings) plus the
        // print-page titles; deeper dynamically-generated strings not yet wrapped in t()
        // will still show in English regardless of the selected language.
        const LANG_STORAGE_KEY = "ledgerAppLanguage";
        const SUPPORTED_LANGS = ["en", "zh-CN", "zh-TW"];
        let currentLang = "en";
        try {
            const savedLang = localStorage.getItem(LANG_STORAGE_KEY);
            if (SUPPORTED_LANGS.includes(savedLang)) currentLang = savedLang;
        } catch (e) {}

        const I18N_ZH_CN = {
            "← Back": "← 返回",
            "(Not set)": "（未设置）",
            "(date you started holding this property — used to show how long it's been held)": "（您开始持有该房产的日期 — 用于显示持有时长）",
            "(e.g. 60, 99, 999)": "（例如 60、99、999）",
            "(manual — from your bank statement)": "（手动 — 来自您的银行对账单）",
            "(optional)": "（选填）",
            "+ Add Account": "+ 添加账户",
            "+ Add Currency": "+ 添加货币",
            "+ Add Placement": "+ 添加存款",
            ...
            "30 min": "30 分钟",
            "5 min": "5 分钟",
            "ACCOUNTS WIDGET": "账户小工具",
            "Account": "账户",
            "Account (transfer from)": "账户（转出）",
            "Account / Reference No. (optional)": "账户 / 参考编号（选填）",
            "Account Ledger": "账户明细",
            "Account Name": "账户名称",
            "Account No. / Ref": "账户编号 / 参考",
            "Account Type": "账户类型",
            "Accounts": "账户",
            "Accounts owned by this member": "该成员名下的账户",
            "Accounts, then Recent Transactions": "先账户，后最近交易",
            "Actual Interest Received": "实际收到利息",
            "Add / Edit Account": "添加 / 编辑账户",
            "Add Category": "添加分类",
            "Add Fund": "添加基金",
            "Add Member": "添加成员",
            "Add Tag": "添加标签",
            ...
            "All Accounts": "所有账户",
            "All Members (everyone, incl. joint)": "所有成员（含联名账户）",
            "All Members (show every account)": "所有成员（显示每个账户）",
            "All Months": "所有月份",
            "All Years": "所有年份",
            "All funds, across every Unit Trust account": "所有单位信托账户中的全部基金",
            "All income & spending categories": "所有收入与支出分类",
            "Amount": "金额",
            "Amount Received": "已收金额",
            "App Local Database": "应用本地数据库",
            "App Local Database footprint: ~0.00 MB (0 attached, 0.00 MB)": "应用本地数据库占用：约 0.00 MB（0 个附件，0.00 MB）",
            "April": "四月",
            "As of Date": "截至日期",
            "Attachments": "附件",
            "Attachments (optional) — photos or PDFs": "附件（选填）— 照片或 PDF",
            "August": "八月",
            "Auto-Lock": "自动锁定",
            "Background Theme": "背景主题",
            "Backup & Restore": "备份与还原",
            "Balanced": "均衡",
            "Balanced Category Sheet": "均衡分类表",
            "Bar Chart": "柱状图",
            "Bond": "债券",
            "Buy": "买入",
            "C": "C",
            "CPF (Singapore)": "CPF（新加坡）",
            "Calculator": "计算器",
            "Cancel": "取消",
            "Cancel Edit": "取消编辑",
            "Categories": "分类",
            "Category": "分类",
            "Category Label Name": "分类标签名称",
            "Change Passcode": "更改密码",
            "Chart Type": "图表类型",
            "Checked": "已选",
            "Clear": "清除",
            "Click a category to filter": "点击分类以筛选",
            "Close": "关闭",
            "Color": "颜色",
            "Commencing Date": "开始日期",
            "Commercial": "商用",
            "Commit Entry": "提交记录",
            "Confirm": "确认",
            "Confirm New Passcode": "确认新密码",
            ...
            "Credit Limit": "信用额度",
            "Currency": "货币",
            "Currency Activity": "货币活动",
            "Current Balance": "当前余额",
            "Current NAV": "当前净值",
            "Current Passcode": "当前密码",
            "Current Redraw Amount": "当前可提取金额",
            "Daily NAV Update": "每日净值更新",
            "Dashboard": "仪表板",
            "Dashboard Widgets": "仪表板小工具",
            "Date": "日期",
            "December": "十二月",
            "Decrypt": "解密",
            ...
            "Default Payment Account": "默认付款账户",
            "Default Receive Account": "默认收款账户",
            "Description Summary": "描述摘要",
            "Design Custom Category": "自定义分类设计",
            "Dividend (Cheque Payout)": "股息（支票发放）",
            "Dividend (Reinvest)": "股息（再投资）",
            "Donut Chart": "环形图",
            "EPF (Malaysia)": "EPF（马来西亚）",
            "EPF / KWSP (EE)": "EPF / KWSP（雇员）",
            "EPF / KWSP (ER)": "EPF / KWSP（雇主）",
            "EPF / KWSP Account": "EPF / KWSP 账户",
            "EPF Employee (EE)": "EPF 雇员部分",
            "EPF Employer (ER)": "EPF 雇主部分",
            "EUR (€)": "EUR（€）",
            "EXCLUDED FROM REPORT": "报告中已排除",
            "EXPENSE CATEGORIES": "支出分类",
            "Enter Passcode": "输入密码",
            "Enter either field — the other calculates automatically from it.": "输入其中一项，另一项将自动计算。",
            "Equity": "股票",
            "Exchange Rate": "汇率",
            "Exchange Rates": "汇率",
            "Exclude if this property's recorded cost shouldn't count toward the Dashboard's Net Worth total.": "如果该房产的记录成本不应计入仪表板的净资产总额，请勾选排除。",
            "Expense": "支出",
            "Expense Breakdown by Category": "按分类的支出明细",
            "Expense Only": "仅支出",
            "Export & Import": "导出与导入",
            "Export & import your data": "导出与导入您的数据",
            "FIXED ASSETS": "固定资产",
            "February": "二月",
            "Fin. Type": "财务类型",
            "Financial Accounts": "财务账户",
            "Financial Assets vs Real Estate": "金融资产 vs 房地产",
            "Financial Report Card": "财务报告卡",
            "Forgot passcode? Reset app data": "忘记密码？重置应用数据",
            "Freehold": "永久产权",
            "Fund": "基金",
            "Fund Activity": "基金活动",
            "Fund Code (optional)": "基金代码（选填）",
            "Fund Detail": "基金详情",
            "Fund Holdings": "基金持仓",
            "Fund Name": "基金名称",
            "GBP (£)": "GBP（£）",
            ...
            "Gross Salary": "总薪资",
            "Group": "组别",
            "Holding Period Start Date": "持有期开始日期",
            "INCOME CATEGORIES": "收入分类",
            "Include in Net Worth (Dashboard)": "计入净资产（仪表板）",
            "Income": "收入",
            ...
            "Industrial": "工业用",
            "Insights": "洞察",
            "Interest Rate (% p.a.)": "利率（年利率 %）",
            "January": "一月",
            "July": "七月",
            "June": "六月",
            "Land": "土地",
            "Lease Expiry": "租约到期日",
            "Lease Term (years)": "租期（年）",
            "Leasehold": "租赁产权",
            "Ledger": "账本",
            "Linked Placement": "关联存款",
            "List (bars + %)": "列表（条形 + 百分比）",
            "Lock App Now": "立即锁定应用",
            "Lock the app automatically after inactivity": "闲置后自动锁定应用",
            "Log Ledger Item": "记录账目",
            "MY NET WORTH": "我的净资产",
            "MYR": "MYR",
            "MYR (RM)": "MYR（RM）",
            "Main Base Currency": "主要基础货币",
            ...
            "Member": "成员",
            "Members": "成员",
            "Members, backups, auto-lock & storage": "成员、备份、自动锁定与存储",
            "Money Market": "货币市场",
            "Monthly Trend": "月度趋势",
            "Multi-Currency Financial Tracker": "多货币财务追踪器",
            "My Ledger": "我的账本",
            "NAV Date:": "净值日期：",
            "Name": "名称",
            "Net": "净额",
            "Net Current Assets": "净流动资产",
            "Net Savings Statement": "净储蓄报表",
            "Net Worth Statement": "净资产报表",
            "Net Worth by Currency": "按货币划分的净资产",
            "Net Worth by Member": "按成员划分的净资产",
            "Net worth split by owner, converted to": "按所有者划分的净资产，已换算为",
            "Never": "从不",
            "New Account / Reference No. (optional)": "新账户 / 参考编号（选填）",
            "New Passcode": "新密码",
            "New Placement Terms": "新存款条款",
            "None (plain salary)": "无（普通薪资）",
            "Not counted in Total Income above.": "不计入上方总收入。",
            "Not counted in Total Spending above.": "不计入上方总支出。",
            "Not counted in the Surplus/Deficit above.": "不计入上方盈余/赤字。",
            "Notes": "备注",
            "November": "十一月",
            "Number of items": "项目数量",
            "Number of items shown": "显示的项目数量",
            "October": "十月",
            "On-device storage footprint": "设备存储占用",
            "Opening Balances (optional)": "期初余额（选填）",
            "Opening Fixed Deposit Placements (optional)": "期初定期存款（选填）",
            "Opening Value": "期初价值",
            "Options": "选项",
            "Order on Dashboard": "仪表板显示顺序",
            "Other": "其他",
            "Overview": "概览",
            "Owner(s)": "所有者",
            "Owner(s) — select one for an individual account, or several for a joint account": "所有者 — 单人账户选一位，联名账户可多选",
            "Parent Category (optional)": "上级分类（选填）",
            "Passcode": "密码",
            "Payment Due Day": "还款到期日",
            "Pick a tag to see its total, breakdown & transactions": "选择标签以查看其总额、明细与交易",
            "Portfolio Net Worth": "投资组合净资产",
            "Portfolio Value": "投资组合价值",
            "Price per Unit": "单位价格",
            "Profit / Loss": "盈亏",
            "RECENT TRANSACTIONS WIDGET": "最近交易小工具",
            "REIT": "房地产投资信托",
            "RM0.00": "RM0.00",
            "Rate (1 unit = ? in account currency)": "汇率（1 单位 = ? 账户货币）",
            "Recent Large Transactions": "近期大额交易",
            "Recent Transactions": "最近交易",
            "Recent Transactions, then Accounts": "先最近交易，后账户",
            "Record Gross Salary → Net Bank + EPF/CPF Split": "记录总薪资 → 净入账 + EPF/CPF 拆分",
            "Register Category Badge": "分类徽章设置",
            "Related Account (optional)": "关联账户（选填）",
            "Renewal Mode": "续存方式",
            "Reports": "报告",
            "Reset App Data": "重置应用数据",
            "Residential": "住宅",
            "Resolution Date": "结算日期",
            "Return": "回报",
            "Reusable presets for the ⭐ picker on Add Income/Expense": "用于「添加收入/支出」⭐ 选择器的可复用预设",
            "Salary": "薪资",
            "Save FX Values": "保存汇率数值",
            "Save Fund": "保存基金",
            "Save Record": "保存记录",
            "Save Tag": "保存标签",
            "Save Template": "保存模板",
            ...
            "Security": "证券",
            "Select Account": "选择账户",
            "Select a Transaction Template": "选择交易模板",
            "Select account": "选择账户",
            "Select category": "选择分类",
            "Select placement": "选择存款",
            "Select...": "请选择...",
            "Selected Badge Icon:": "已选徽章图标：",
            "Sell": "卖出",
            "September": "九月",
            "Set an App Passcode": "设置应用密码",
            "Setting": "设置",
            "Show": "显示",
            "Show All Accounts": "显示所有账户",
            "Spending Breakdown": "支出明细",
            "Spending Breakdown (This Month)": "支出明细（本月）",
            "Spending by Tag": "按标签划分的支出",
            "Split Total:": "拆分总额：",
            "Statement Day": "对账单日",
            "Sub-Group (optional)": "子分组（选填）",
            "Surplus Margin:": "盈余幅度：",
            "Tag": "标签",
            "Tag Name": "标签名称",
            "Tag accounts to a member — husband, wife, kids, etc.": "将账户标记给某位成员 — 如丈夫、妻子、孩子等",
            "Tags": "标签",
            "Tap an account to view its activity": "点击账户以查看其活动",
            "Tap to see Net Worth by Currency Held": "点击查看按持有货币划分的净资产",
            "Template Name": "模板名称",
            "Tenure": "期限",
            "Tenure (months)": "期限（月）",
            "This Month": "本月",
            "This Year": "今年",
            "This backup is encrypted. Enter its passcode to restore it.": "此备份已加密。请输入密码以还原。",
            "This ledger is locked. Enter your passcode to decrypt and continue.": "账本已锁定。请输入密码解密后继续。",
            "To / Payee": "收款方",
            "Top Categories This Month": "本月热门分类",
            "Total": "总计",
            "Total Amount": "总金额",
            "Total Assets": "总资产",
            "Total Assets − Total Liabilities, before Real Estate below.": "总资产 − 总负债（不含下方房地产）。",
            "Total Excluded": "已排除总额",
            "Total Expenses": "总支出",
            "Total Fixed Assets": "固定资产总额",
            "Total Income": "总收入",
            "Total Invested": "总投资额",
            "Total Liabilities": "总负债",
            "Total browser quota limit: ~10240.00 MB, Used: 0.00 MB (across local sandboxes)": "浏览器配额上限：约 10240.00 MB，已用：0.00 MB（跨本地沙盒）",
            "Transaction Log History (Click item to Edit)": "交易记录历史（点击可编辑）",
            "Transaction Templates": "交易模板",
            "Transaction Type": "交易类型",
            "Transactions": "交易",
            "Transfer Interest To": "利息转入",
            "Transfer Proceeds To": "款项转入",
            "Transfer money": "转账",
            "Trip/claim labels for the Add Income/Expense form's Tags field": "「添加收入/支出」表单标签字段的行程/报销标签",
            "Type": "类型",
            "USD": "USD",
            "USD ($)": "USD（$）",
            "Unit Trust Portfolio": "单位信托投资组合",
            "Units": "单位数",
            "Unlock": "解锁",
            "Update All Prices": "更新所有价格",
            "Update unit prices for funds you currently hold": "更新您当前持有基金的单位价格",
            "Use This Value": "使用此数值",
            "WHAT I HAVE": "我拥有的",
            "WHAT I OWE": "我欠的",
            "What would you like to do?": "您想做什么？",
            "What you have, what you owe, and what's left": "您拥有的、您欠的，以及剩余的",
            "What you hold in each currency, native amounts": "您在每种货币中持有的原始金额",
            "Which account the 💳 Pay button defaults to paying from — you can still change it per payment.": "💳「付款」按钮默认扣款的账户 — 每次付款时仍可更改。",
            "Which account this loan relates to — e.g. the bank account repayments come from.": "此贷款关联的账户 — 例如还款所从的银行账户。",
            "Which part?": "哪一部分？",
            "base currency": "基础货币",
            "so funds in different currencies can be totalled together.": "以便不同货币的资金可以汇总计算。",
            "— None (Main Category) —": "— 无（主分类）—",
            "— Select a tag —": "— 选择标签 —",
            "→ Bank Account": "→ 银行账户",
            "↩️ Refund": "↩️ 退款",
            "⏱️ Auto-lock after inactivity": "⏱️ 闲置后自动锁定",
            "▢ Card": "▢ 卡片",
            "⚠️ Reset App Data": "⚠️ 重置应用数据",
            "✅ Include": "✅ 包含",
            "✈️ Overseas Allowance": "✈️ 海外津贴",
            "✏️ Edit transaction": "✏️ 编辑交易",
            "✏️ Manually enter FX rate": "✏️ 手动输入汇率",
            "✏️ Manually fill Description": "✏️ 手动填写描述",
            "✏️ Manually set conversion": "✏️ 手动设置换算",
            "✏️ 蠟筆小新: Handwritten font (Kalam)": "✏️ 蜡笔小新：手写字体（Kalam）",
            "➕ Renew with Interest": "➕ 续存并计息",
            "⤢ Auto-scale": "⤢ 自动缩放",
            "⬇ Download": "⬇ 下载",
            "🌗 Auto (follow system day/night)": "🌗 自动（跟随系统日夜模式）",
            "🏛️ Normal": "🏛️ 普通",
            "🏛️ Principal Only": "🏛️ 仅本金",
            "🏠 Housing Allowance": "🏠 住房津贴",
            "🏠 Real Estate": "🏠 房地产",
            "🏦 Bank/Cash": "🏦 银行/现金",
            "🏦 Fixed Deposit": "🏦 定期存款",
            "🏦 Fixed Deposit Placement Terms": "🏦 定期存款条款",
            "🏦 Resolve Maturity": "🏦 处理到期",
            "🏦 This is interest paid out from an FD placement": "🏦 这是定期存款支付的利息",
            "💰 Salary Entry": "💰 薪资记录",
            "💱 Multi-Currency": "💱 多货币",
            "💳 Amount due:": "💳 应付金额：",
            "💳 Credit Card": "💳 信用卡",
            ...
            "📜 History": "📜 历史记录",
            "📥 Import JSON": "📥 导入 JSON",
            "📱 Handphone Claim": "📱 手机报销",
            "📷 Take Photo": "📷 拍照",
            "🔁 Currency Conversion": "🔁 货币换算",
            "🔄 Fetch Live Rates": "🔄 获取实时汇率",
            "🔄 Renew": "🔄 续存",
            "🔐 Encrypt this backup export": "🔐 加密此备份导出",
            "🔑 Change Passcode": "🔑 更改密码",
            "🔴 Expense": "🔴 支出",
            "🔴 Expense Templates": "🔴 支出模板",
            ...
            "🗑 Delete Member": "🗑 删除成员",
            "🗑 Delete Transaction": "🗑 删除交易",
            "🗑️ Erase Everything": "🗑️ 清除所有数据",
            "🚫 Exclude": "🚫 排除",
            "🟢 Income": "🟢 收入",
            "🟢 Income Categories": "🟢 收入分类",
            "🟢 Income Templates": "🟢 收入模板",
            "🧺 Unit Trust": "🧺 单位信托",
            "🧾 Other Liabilities": "🧾 其他负债",
            "🫆 Biometric quick unlock (fingerprint/Face)": "🫆 生物识别快速解锁（指纹/人脸）",
            ...
            "Back to top": "回到顶部",
            "Backpay": "补发工资",
            "Backpay Note (Optional)": "补发说明（选填）",
            "Backpay Note (Optional) — e.g. Nov 2025 – Jan 2026 salary revision": "补发说明（选填）— 例如 2025年11月至2026年1月薪资调整",
            "Backup passcode": "备份密码",
            "Bank Account": "银行账户",
            "Base currency": "基础货币",
            "Calculator / Numpad": "计算器 / 数字键盘",
            "Close insights panel": "关闭洞察面板",
            "Collapse account type list": "收起账户类型列表",
            "Collapse or expand this section": "收起或展开此部分",
            "Confirm new passcode": "确认新密码",
            "Confirm passcode": "确认密码",
            "Create passcode (min 4 characters)": "创建密码（至少 4 位）",
            "Current passcode": "当前密码",
            "Description": "描述",
            "Edit account credentials": "编辑账户信息",
            "Edit fund details": "编辑基金详情",
            "Enter your passcode": "输入您的密码",
            "Expand fund detail": "展开基金详情",
            ...
            "Lock app now": "立即锁定应用",
            "More options": "更多选项",
            "New passcode (min 4 characters)": "新密码（至少 4 位）",
            "Notes (Optional)": "备注（选填）",
            "Number of Items": "项目数量",
            "Number of Items Shown": "显示的项目数量",
            "Open menu": "打开菜单",
            "Optional notes": "选填备注",
            "Overseas Allowance": "海外津贴",
            "Print this page": "打印此页面",
            "Quick export (encrypted backup)": "快速导出（加密备份）",
            "Quick export encrypted backup": "快速导出加密备份",
            "Re-enter your passcode": "重新输入您的密码",
            "Rescale the axis to the second-highest bar so one outlier month doesn't flatten the rest — the outlier bar still shows, just clipped with a notch; tap/hover it for its real value.": "将坐标轴缩放至次高柱形，避免单个异常月份压扁其余数据 — 异常柱形仍会显示，只是以缺口方式截断；点击/悬停可查看其真实数值。",
            "Select Bank Account": "选择银行账户",
            "Select Category": "选择分类",
            "Select Currency": "选择货币",
            "Select Destination Account": "选择目标账户",
            "Select EPF/CPF Account": "选择 EPF/CPF 账户",
            "Select Fixed Deposit Placement": "选择定期存款",
            "Select Member": "选择成员",
            "Select Scheme": "选择计划",
            "Split into another category": "拆分至其他分类",
            "Tap for the full Net Worth Statement (assets vs. liabilities breakdown)": "点击查看完整净资产报表（资产与负债明细）",
            "Tap to reveal": "点击显示",
            "To Account": "目标账户",
            "Toggle privacy mode": "切换隐私模式",
            "Toggle privacy mode (blur amounts)": "切换隐私模式（模糊金额）",
            "Use a transaction template": "使用交易模板",
            "e.g. 038-68-02838-0": "例如 038-68-02838-0",
            "e.g. 99": "例如 99",
            "e.g. August 2026 Salary": "例如 2026年8月薪资",
            "e.g. DBS Bank, Cash Wallet": "例如 DBS银行、现金钱包",
            "e.g. FD-2026-00123": "例如 FD-2026-00123",
            "e.g. FD-2026-00456": "例如 FD-2026-00456",
            "e.g. Home Internet, Salary": "例如 家庭网络、薪资",
            "e.g. Husband, Wife, Alex": "例如 丈夫、妻子、Alex",
            "e.g. Japan Holiday Aug 2026": "例如 2026年8月日本假期",
            "e.g. Subscriptions, Crypto": "例如 订阅服务、加密货币",
            "e.g. UMobile Home 5G": "例如 UMobile 家用5G",
            "e.g., GEF001": "例如 GEF001",
            "e.g., Global Equity Fund": "例如 全球股票基金",
            "Language": "语言",
            "Database": "数据库",
            "Dashboard — Net Worth Overview": "仪表板 — 净资产概览",
            "Auto-Lock Settings": "自动锁定设置",
            "Savings Statement": "储蓄报表",
            "Settings": "设置",
        };
        ...
        const I18N_ZH_TW = {
            "← Back": "← 返回",
            "(Not set)": "（未設置）",
            "(date you started holding this property — used to show how long it's been held)": "（您開始持有該房產的日期 — 用於顯示持有時長）",
            "(e.g. 60, 99, 999)": "（例如 60、99、999）",
            "(manual — from your bank statement)": "（手動 — 來自您的銀行對帳單）",
            "(optional)": "（選填）",
            "+ Add Account": "+ 添加帳戶",
            "+ Add Currency": "+ 添加貨幣",
            "+ Add Placement": "+ 添加存款",
            ...
            "30 min": "30 分鐘",
            "5 min": "5 分鐘",
            "ACCOUNTS WIDGET": "帳戶小工具",
            "Account": "帳戶",
            "Account (transfer from)": "帳戶（轉出）",
            "Account / Reference No. (optional)": "帳戶 / 參考編號（選填）",
            "Account Ledger": "帳戶明細",
            "Account Name": "帳戶名稱",
            "Account No. / Ref": "帳戶編號 / 參考",
            "Account Type": "帳戶類型",
            "Accounts": "帳戶",
            "Accounts owned by this member": "該成員名下的帳戶",
            "Accounts, then Recent Transactions": "先帳戶，後最近交易",
            "Actual Interest Received": "實際收到利息",
            "Add / Edit Account": "添加 / 編輯帳戶",
            "Add Category": "添加分類",
            "Add Fund": "添加基金",
            "Add Member": "添加成員",
            "Add Tag": "添加標籤",
            ...
            "Add Transaction": "添加交易",
            "All Accounts": "所有帳戶",
            "All Members (everyone, incl. joint)": "所有成員（含聯名帳戶）",
            "All Members (show every account)": "所有成員（顯示每個帳戶）",
            "All Months": "所有月份",
            "All Years": "所有年份",
            "All funds, across every Unit Trust account": "所有單位信託帳戶中的全部基金",
            "All income & spending categories": "所有收入與支出分類",
            "Amount": "金額",
            "Amount Received": "已收金額",
            "App Local Database": "應用本地數據庫",
            "App Local Database footprint: ~0.00 MB (0 attached, 0.00 MB)": "應用本地數據庫佔用：約 0.00 MB（0 個附件，0.00 MB）",
            "April": "四月",
            "As of Date": "截至日期",
            "Attachments": "附件",
            "Attachments (optional) — photos or PDFs": "附件（選填）— 照片或 PDF",
            "August": "八月",
            "Auto-Lock": "自動鎖定",
            "Background Theme": "背景主題",
            "Backup & Restore": "備份與還原",
            "Balanced": "均衡",
            "Balanced Category Sheet": "均衡分類表",
            "Bar Chart": "柱狀圖",
            "Bond": "債券",
            "Buy": "買入",
            "C": "C",
            ...
            "Categories": "分類",
            "Category": "分類",
            "Category Label Name": "分類標籤名稱",
            "Change Passcode": "更改密碼",
            "Chart Type": "圖表類型",
            "Checked": "已選",
            "Clear": "清除",
            "Click a category to filter": "點擊分類以篩選",
            "Close": "關閉",
            "Color": "顏色",
            "Commencing Date": "開始日期",
            "Commercial": "商用",
            "Commit Entry": "提交記錄",
            "Confirm": "確認",
            "Confirm New Passcode": "確認新密碼",
            "Confirm Passcode": "確認密碼",
            "Contribution": "供款",
            "Create & Continue": "創建並繼續",
            "Create Account": "創建帳戶",
            "Create New Account": "創建新帳戶",
            "Credit Limit": "信用額度",
            "Currency": "貨幣",
            "Currency Activity": "貨幣活動",
            "Current Balance": "當前餘額",
            "Current NAV": "當前淨值",
            "Current Passcode": "當前密碼",
            "Current Redraw Amount": "當前可提取金額",
            "Daily NAV Update": "每日淨值更新",
            "Dashboard": "儀表板",
            "Dashboard Widgets": "儀表板小工具",
            "Date": "日期",
            "December": "十二月",
            "Decrypt": "解密",
            "Default Accounts": "默認帳戶",
            "Default Category": "默認分類",
            "Default Expense Category": "默認支出分類",
            "Default Income Category": "默認收入分類",
            "Default Payment Account": "默認付款帳戶",
            "Default Receive Account": "默認收款帳戶",
            "Description Summary": "描述摘要",
            "Design Custom Category": "自定義分類設計",
            "Dividend (Cheque Payout)": "股息（支票發放）",
            "Dividend (Reinvest)": "股息（再投資）",
            "Donut Chart": "環形圖",
            "EPF (Malaysia)": "EPF（馬來西亞）",
            "EPF / KWSP (EE)": "EPF / KWSP（僱員）",
            "EPF / KWSP (ER)": "EPF / KWSP（僱主）",
            "EPF / KWSP Account": "EPF / KWSP 帳戶",
            "EPF Employee (EE)": "EPF 僱員部分",
            "EPF Employer (ER)": "EPF 僱主部分",
            "EUR (€)": "EUR（€）",
            "EXCLUDED FROM REPORT": "報告中已排除",
            "EXPENSE CATEGORIES": "支出分類",
            "Enter Passcode": "輸入密碼",
            "Enter either field — the other calculates automatically from it.": "輸入其中一項，另一項將自動計算。",
            "Equity": "股票",
            "Exchange Rate": "匯率",
            "Exchange Rates": "匯率",
            "Exclude if this property's recorded cost shouldn't count toward the Dashboard's Net Worth total.": "如果該房產的記錄成本不應計入儀表板的淨資產總額，請勾選排除。",
            "Expense": "支出",
            "Expense Breakdown by Category": "按分類的支出明細",
            "Expense Only": "僅支出",
            "Export & Import": "導出與導入",
            "Export & import your data": "導出與導入您的數據",
            "FIXED ASSETS": "固定資產",
            "February": "二月",
            "Fin. Type": "財務類型",
            "Financial Accounts": "財務帳戶",
            "Financial Assets vs Real Estate": "金融資產 vs 房地產",
            "Financial Report Card": "財務報告卡",
            "Forgot passcode? Reset app data": "忘記密碼？重置應用數據",
            "Freehold": "永久產權",
            "Fund": "基金",
            "Fund Activity": "基金活動",
            "Fund Code (optional)": "基金代碼（選填）",
            "Fund Detail": "基金詳情",
            "Fund Holdings": "基金持倉",
            "Fund Name": "基金名稱",
            "GBP (£)": "GBP（£）",
            "Global Currency Settings": "全局貨幣設置",
            "Gross Salary": "總薪資",
            "Group": "組別",
            "Holding Period Start Date": "持有期開始日期",
            "INCOME CATEGORIES": "收入分類",
            "Include in Net Worth (Dashboard)": "計入淨資產（儀表板）",
            "Income": "收入",
            "Income + Expense": "收入 + 支出",
            "Income Breakdown": "收入明細",
            "Income Only": "僅收入",
            "Industrial": "工業用",
            "Insights": "洞察",
            "Interest Rate (% p.a.)": "利率（年利率 %）",
            ...
            "Leasehold": "租賃產權",
            "Ledger": "帳本",
            "Linked Placement": "關聯存款",
            "List (bars + %)": "列表（條形 + 百分比）",
            "Lock App Now": "立即鎖定應用",
            "Lock the app automatically after inactivity": "閒置後自動鎖定應用",
            "Log Ledger Item": "記錄帳目",
            "MY NET WORTH": "我的淨資產",
            "MYR": "MYR",
            "MYR (RM)": "MYR（RM）",
            "Main Base Currency": "主要基礎貨幣",
            ...
            "Member": "成員",
            "Members": "成員",
            "Members, backups, auto-lock & storage": "成員、備份、自動鎖定與存儲",
            "Money Market": "貨幣市場",
            "Monthly Trend": "月度趨勢",
            "Multi-Currency Financial Tracker": "多貨幣財務追蹤器",
            "My Ledger": "我的帳本",
            "NAV Date:": "淨值日期：",
            "Name": "名稱",
            "Net": "淨額",
            "Net Current Assets": "淨流動資產",
            "Net Savings Statement": "淨儲蓄報表",
            "Net Worth Statement": "淨資產報表",
            "Net Worth by Currency": "按貨幣劃分的淨資產",
            "Net Worth by Member": "按成員劃分的淨資產",
            "Net worth split by owner, converted to": "按所有者劃分的淨資產，已換算為",
            "Never": "從不",
            "New Account / Reference No. (optional)": "新帳戶 / 參考編號（選填）",
            "New Passcode": "新密碼",
            "New Placement Terms": "新存款條款",
            "None (plain salary)": "無（普通薪資）",
            "Not counted in Total Income above.": "不計入上方總收入。",
            "Not counted in Total Spending above.": "不計入上方總支出。",
            "Not counted in the Surplus/Deficit above.": "不計入上方盈餘/赤字。",
            "Notes": "備註",
            "November": "十一月",
            "Number of items": "項目數量",
            "Number of items shown": "顯示的項目數量",
            "October": "十月",
            "On-device storage footprint": "設備存儲佔用",
            "Opening Balances (optional)": "期初餘額（選填）",
            "Opening Fixed Deposit Placements (optional)": "期初定期存款（選填）",
            "Opening Value": "期初價值",
            "Options": "選項",
            "Order on Dashboard": "儀表板顯示順序",
            "Other": "其他",
            "Overview": "概覽",
            "Owner(s)": "所有者",
            "Owner(s) — select one for an individual account, or several for a joint account": "所有者 — 單人帳戶選一位，聯名帳戶可多選",
            "Parent Category (optional)": "上級分類（選填）",
            "Passcode": "密碼",
            "Payment Due Day": "還款到期日",
            "Pick a tag to see its total, breakdown & transactions": "選擇標籤以查看其總額、明細與交易",
            "Portfolio Net Worth": "投資組合淨資產",
            ...
            "REIT": "房地產投資信託",
            "RM0.00": "RM0.00",
            "Rate (1 unit = ? in account currency)": "匯率（1 單位 = ? 帳戶貨幣）",
            "Recent Large Transactions": "近期大額交易",
            "Recent Transactions": "最近交易",
            "Recent Transactions, then Accounts": "先最近交易，後帳戶",
            "Record Gross Salary → Net Bank + EPF/CPF Split": "記錄總薪資 → 淨入帳 + EPF/CPF 拆分",
            "Register Category Badge": "分類徽章設置",
            "Related Account (optional)": "關聯帳戶（選填）",
            "Renewal Mode": "續存方式",
            "Reports": "報告",
            "Reset App Data": "重置應用數據",
            "Residential": "住宅",
            "Resolution Date": "結算日期",
            "Return": "回報",
            "Reusable presets for the ⭐ picker on Add Income/Expense": "用於「添加收入/支出」⭐ 選擇器的可複用預設",
            "Salary": "薪資",
            "Save FX Values": "保存匯率數值",
            "Save Fund": "保存基金",
            "Save Record": "保存記錄",
            "Save Tag": "保存標籤",
            "Save Template": "保存模板",
            ...
            "Security": "證券",
            "Select Account": "選擇帳戶",
            "Select a Transaction Template": "選擇交易模板",
            "Select account": "選擇帳戶",
            "Select category": "選擇分類",
            "Select placement": "選擇存款",
            "Select...": "請選擇...",
            "Selected Badge Icon:": "已選徽章圖標：",
            "Sell": "賣出",
            "September": "九月",
            "Set an App Passcode": "設置應用密碼",
            "Setting": "設置",
            "Show": "顯示",
            "Show All Accounts": "顯示所有帳戶",
            "Spending Breakdown": "支出明細",
            "Spending Breakdown (This Month)": "支出明細（本月）",
            "Spending by Tag": "按標籤劃分的支出",
            "Split Total:": "拆分總額：",
            "Statement Day": "對帳單日",
            "Sub-Group (optional)": "子分組（選填）",
            "Surplus Margin:": "盈餘幅度：",
            "Tag": "標籤",
            "Tag Name": "標籤名稱",
            "Tag accounts to a member — husband, wife, kids, etc.": "將帳戶標記給某位成員 — 如丈夫、妻子、孩子等",
            "Tags": "標籤",
            "Tap an account to view its activity": "點擊帳戶以查看其活動",
            "Tap to see Net Worth by Currency Held": "點擊查看按持有貨幣劃分的淨資產",
            "Template Name": "模板名稱",
            "Tenure": "期限",
            "Tenure (months)": "期限（月）",
            "This Month": "本月",
            "This Year": "今年",
            "This backup is encrypted. Enter its passcode to restore it.": "此備份已加密。請輸入密碼以還原。",
            "This ledger is locked. Enter your passcode to decrypt and continue.": "帳本已鎖定。請輸入密碼解密後繼續。",
            "To / Payee": "收款方",
            "Top Categories This Month": "本月熱門分類",
            "Total": "總計",
            "Total Amount": "總金額",
            "Total Assets": "總資產",
            "Total Assets − Total Liabilities, before Real Estate below.": "總資產 − 總負債（不含下方房地產）。",
            "Total Excluded": "已排除總額",
            "Total Expenses": "總支出",
            "Total Fixed Assets": "固定資產總額",
            "Total Income": "總收入",
            "Total Invested": "總投資額",
            "Total Liabilities": "總負債",
            "Total browser quota limit: ~10240.00 MB, Used: 0.00 MB (across local sandboxes)": "瀏覽器配額上限：約 10240.00 MB，已用：0.00 MB（跨本地沙盒）",
            "Transaction Log History (Click item to Edit)": "交易記錄歷史（點擊可編輯）",
            "Transaction Templates": "交易模板",
            "Transaction Type": "交易類型",
            "Transactions": "交易",
            "Transfer Interest To": "利息轉入",
            "Transfer Proceeds To": "款項轉入",
            "Transfer money": "轉帳",
            "Trip/claim labels for the Add Income/Expense form's Tags field": "「添加收入/支出」表單標籤字段的行程/報銷標籤",
            "Type": "類型",
            "USD": "USD",
            "USD ($)": "USD（$）",
            "Unit Trust Portfolio": "單位信託投資組合",
            "Units": "單位數",
            "Unlock": "解鎖",
            "Update All Prices": "更新所有價格",
            "Update unit prices for funds you currently hold": "更新您當前持有基金的單位價格",
            "Use This Value": "使用此數值",
            "WHAT I HAVE": "我擁有的",
            "WHAT I OWE": "我欠的",
            "What would you like to do?": "您想做什麼？",
            "What you have, what you owe, and what's left": "您擁有的、您欠的，以及剩餘的",
            "What you hold in each currency, native amounts": "您在每種貨幣中持有的原始金額",
            "Which account the 💳 Pay button defaults to paying from — you can still change it per payment.": "💳「付款」按鈕默認扣款的帳戶 — 每次付款時仍可更改。",
            "Which account this loan relates to — e.g. the bank account repayments come from.": "此貸款關聯的帳戶 — 例如還款所從的銀行帳戶。",
            "Which part?": "哪一部分？",
            "base currency": "基礎貨幣",
            "so funds in different currencies can be totalled together.": "以便不同貨幣的資金可以彙總計算。",
            "— None (Main Category) —": "— 無（主分類）—",
            "— Select a tag —": "— 選擇標籤 —",
            "→ Bank Account": "→ 銀行帳戶",
            "↩️ Refund": "↩️ 退款",
            "⏱️ Auto-lock after inactivity": "⏱️ 閒置後自動鎖定",
            "▢ Card": "▢ 卡片",
            "⚠️ Reset App Data": "⚠️ 重置應用數據",
            "✅ Include": "✅ 包含",
            "✈️ Overseas Allowance": "✈️ 海外津貼",
            "✏️ Edit transaction": "✏️ 編輯交易",
            "✏️ Manually enter FX rate": "✏️ 手動輸入匯率",
            "✏️ Manually fill Description": "✏️ 手動填寫描述",
            "✏️ Manually set conversion": "✏️ 手動設置換算",
            "✏️ 蠟筆小新: Handwritten font (Kalam)": "✏️ 蠟筆小新：手寫字體（Kalam）",
            "➕ Renew with Interest": "➕ 續存並計息",
            "⤢ Auto-scale": "⤢ 自動縮放",
            "⬇ Download": "⬇ 下載",
            "🌗 Auto (follow system day/night)": "🌗 自動（跟隨系統日夜模式）",
            "🏛️ Normal": "🏛️ 普通",
            "🏛️ Principal Only": "🏛️ 僅本金",
            "🏠 Housing Allowance": "🏠 住房津貼",
            "🏠 Real Estate": "🏠 房地產",
            "🏦 Bank/Cash": "🏦 銀行/現金",
            "🏦 Fixed Deposit": "🏦 定期存款",
            "🏦 Fixed Deposit Placement Terms": "🏦 定期存款條款",
            "🏦 Resolve Maturity": "🏦 處理到期",
            "🏦 This is interest paid out from an FD placement": "🏦 這是定期存款支付的利息",
            "💰 Salary Entry": "💰 薪資記錄",
            "💱 Multi-Currency": "💱 多貨幣",
            "💳 Amount due:": "💳 應付金額：",
            "💳 Credit Card": "💳 信用卡",
            "💳 Pay": "💳 付款",
            "💸 Bank Loan": "💸 銀行貸款",
            "💸 Withdraw & Close": "💸 提取並關閉",
            "💼 Financial Assets": "💼 金融資產",
            "💼 Other Assets": "💼 其他資產",
            "💾 Export JSON": "💾 導出 JSON",
            "📂 Duplicate transaction": "📂 複製交易",
            "📅 Calendar": "📅 日曆",
            "📈 Investment": "📈 投資",
            "📋 List": "📋 列表",
            "📋 Table": "📋 表格",
            ...
            "📜 History": "📜 歷史記錄",
            "📥 Import JSON": "📥 導入 JSON",
            "📱 Handphone Claim": "📱 手機報銷",
            "📷 Take Photo": "📷 拍照",
            "🔁 Currency Conversion": "🔁 貨幣換算",
            "🔄 Fetch Live Rates": "🔄 獲取實時匯率",
            "🔄 Renew": "🔄 續存",
            "🔐 Encrypt this backup export": "🔐 加密此備份導出",
            "🔑 Change Passcode": "🔑 更改密碼",
            "🔴 Expense": "🔴 支出",
            "🔴 Expense Templates": "🔴 支出模板",
            "🔴 Spending Categories": "🔴 支出分類",
            "🕒 Backpay": "🕒 補發工資",
            ...
            "🫆 Biometric quick unlock (fingerprint/Face)": "🫆 生物識別快速解鎖（指紋/人臉）",
            "🫆 Unlock with Biometrics": "🫆 使用生物識別解鎖",
            "SGD (S$)": "SGD（S$）",
            "↩️  Refund": "↩️  退款",
            "✏️  Edit transaction": "✏️  編輯交易",
            "📂  Duplicate transaction": "📂  複製交易",
            ...
            "Back to top": "回到頂部",
            "Backpay": "補發工資",
            "Backpay Note (Optional)": "補發說明（選填）",
            "Backpay Note (Optional) — e.g. Nov 2025 – Jan 2026 salary revision": "補發說明（選填）— 例如 2025年11月至2026年1月薪資調整",
            "Backup passcode": "備份密碼",
            "Bank Account": "銀行帳戶",
            "Base currency": "基礎貨幣",
            "Calculator / Numpad": "計算器 / 數字鍵盤",
            "Close insights panel": "關閉洞察面板",
            "Collapse account type list": "收起帳戶類型列表",
            "Collapse or expand this section": "收起或展開此部分",
            "Confirm new passcode": "確認新密碼",
            "Confirm passcode": "確認密碼",
            "Create passcode (min 4 characters)": "創建密碼（至少 4 位）",
            "Current passcode": "當前密碼",
            "Description": "描述",
            "Edit account credentials": "編輯帳戶信息",
            "Edit fund details": "編輯基金詳情",
            "Enter your passcode": "輸入您的密碼",
            "Expand fund detail": "展開基金詳情",
            "Group by description": "按描述分組",
            "Handphone Claim": "手機報銷",
            "Housing Allowance": "住房津貼",
            "Jump to today": "跳轉至今天",
            "Lock app now": "立即鎖定應用",
            "More options": "更多選項",
            "New passcode (min 4 characters)": "新密碼（至少 4 位）",
            "Notes (Optional)": "備註（選填）",
            "Number of Items": "項目數量",
            "Number of Items Shown": "顯示的項目數量",
            "Open menu": "打開菜單",
            "Optional notes": "選填備註",
            "Overseas Allowance": "海外津貼",
            "Print this page": "打印此頁面",
            "Quick export (encrypted backup)": "快速導出（加密備份）",
            "Quick export encrypted backup": "快速導出加密備份",
            "Re-enter your passcode": "重新輸入您的密碼",
            "Rescale the axis to the second-highest bar so one outlier month doesn't flatten the rest — the outlier bar still shows, just clipped with a notch; tap/hover it for its real value.": "將座標軸縮放至次高柱形，避免單個異常月份壓扁其餘數據 — 異常柱形仍會顯示，只是以缺口方式截斷；點擊/懸停可查看其真實數值。",
            "Select Bank Account": "選擇銀行帳戶",
            "Select Category": "選擇分類",
            "Select Currency": "選擇貨幣",
            "Select Destination Account": "選擇目標帳戶",
            "Select EPF/CPF Account": "選擇 EPF/CPF 帳戶",
            "Select Fixed Deposit Placement": "選擇定期存款",
            "Select Member": "選擇成員",
            "Select Scheme": "選擇計劃",
            "Split into another category": "拆分至其他分類",
            "Tap for the full Net Worth Statement (assets vs. liabilities breakdown)": "點擊查看完整淨資產報表（資產與負債明細）",
            "Tap to reveal": "點擊顯示",
            "To Account": "目標帳戶",
            "Toggle privacy mode": "切換隱私模式",
            "Toggle privacy mode (blur amounts)": "切換隱私模式（模糊金額）",
            "Use a transaction template": "使用交易模板",
            "e.g. 038-68-02838-0": "例如 038-68-02838-0",
            "e.g. 99": "例如 99",
            "e.g. August 2026 Salary": "例如 2026年8月薪資",
            "e.g. DBS Bank, Cash Wallet": "例如 DBS銀行、現金錢包",
            "e.g. FD-2026-00123": "例如 FD-2026-00123",
            "e.g. FD-2026-00456": "例如 FD-2026-00456",
            "e.g. Home Internet, Salary": "例如 家庭網絡、薪資",
            "e.g. Husband, Wife, Alex": "例如 丈夫、妻子、Alex",
            "e.g. Japan Holiday Aug 2026": "例如 2026年8月日本假期",
            "e.g. Subscriptions, Crypto": "例如 訂閱服務、加密貨幣",
            "e.g. UMobile Home 5G": "例如 UMobile 家用5G",
            "e.g., GEF001": "例如 GEF001",
            "e.g., Global Equity Fund": "例如 全球股票基金",
            "Language": "語言",
            "Database": "數據庫",
            "Dashboard — Net Worth Overview": "儀表板 — 淨資產概覽",
            "Auto-Lock Settings": "自動鎖定設置",
            "Savings Statement": "儲蓄報表",
            "Settings": "設置",
        };
        ...
        function t(enText) {
            ...
            const dict = currentLang === "zh-CN" ? I18N_ZH_CN : (currentLang === "zh-TW" ? I18N_ZH_TW : null);
            if (!dict) return enText;
            return Object.prototype.hasOwnProperty.call(dict, enText) ? dict[enText] : enText;
        }

        const i18nOrigText = new WeakMap();
        const i18nOrigAttr = new WeakMap();

        // Applies the current language to every element marked up for translation in
        // index.html. Safe to call repeatedly (boot, language switch, or after any code
        // re-shows/rebuilds a chunk of static markup) — original English is cached per
        // element the first time it's seen, never re-derived from already-translated text.
        function applyI18nStatic(root) {
            root = root || document;
            root.querySelectorAll("[data-i18n]").forEach(el => {
                let orig = i18nOrigText.get(el);
                if (orig === undefined) { orig = el.textContent; i18nOrigText.set(el, orig); }
                const leadMatch = orig.match(/^\s*/);
                const trailMatch = orig.match(/\s*$/);
                const lead = leadMatch ? leadMatch[0] : "";
                const trail = trailMatch ? trailMatch[0] : "";
                const core = orig.slice(lead.length, trail.length ? orig.length - trail.length : undefined);
                el.textContent = lead + t(core) + trail;
            });
            const attrMap = [
                ["placeholder", "data-i18n-placeholder"],
                ["title", "data-i18n-title"],
                ["aria-label", "data-i18n-aria-label"],
                ["data-title", "data-i18n-datatitle"],
            ];
            attrMap.forEach(([attr, marker]) => {
                root.querySelectorAll(`[${marker}]`).forEach(el => {
                    let map = i18nOrigAttr.get(el);
                    if (!map) { map = {}; i18nOrigAttr.set(el, map); }
        ...
        // Persists the choice and reloads. A full reload (rather than trying to re-render
        // every currently-mounted dynamic page/modal by hand) is deliberate here — this
        // app already requires re-entering the passcode (or biometric) on every fresh
        // load, so a reload after a rarely-changed setting like language is consistent
        // with normal app-restart behavior and guarantees every page — including ones not
        // currently visible — rebuilds correctly in the new language next time it's opened.
        ...
        async function handleAppLanguageChange() {
            const sel = document.getElementById("appLanguageSelect");
            const lang = (sel && SUPPORTED_LANGS.includes(sel.value)) ? sel.value : "en";
            setAppLanguage(lang);
        }

        // Syncs the Settings > Language picker's selected value + button label to
        // currentLang. Called once on boot (immediate-run block below) since the Setting
        // page's markup exists in the DOM from first paint even though it's hidden.
        function initAppLanguageSelect() {
            const sel = document.getElementById("appLanguageSelect");
            if (!sel) return;
            sel.value = currentLang;
            if (typeof syncAccountPickerButtonText === "function") {
                syncAccountPickerButtonText("appLanguageSelect");
            } else {
                const btnText = document.getElementById("appLanguageSelectBtnText");
                if (btnText) {
                    const opt = sel.options[sel.selectedIndex];
                    if (opt) btnText.textContent = opt.textContent;
                }
            }
        }
        ...
        // v264: apply the saved language to the static shell right away — deliberately here,
        // not inside bootstrap()/the "load" listener, since the lock/setup screen's own text
        // ("Enter Passcode" etc.) needs to already be translated before that screen shows.
        applyI18nStatic(document);
        initAppLanguageSelect();
        ...
        function getActivePageTitle(pageId) {
            switch (pageId) {
                case "page-workspace": return t("Dashboard — Net Worth Overview");
                case "page-ledger": return document.getElementById("ledgerTargetTitle")?.textContent || t("Account Ledger");
                case "page-fundactivity": return document.getElementById("fundActivityTitle")?.textContent || t("Fund Activity");
                case "page-currencyactivity": return document.getElementById("currencyActivityTitle")?.textContent || t("Currency Activity");
                case "page-accounts": return t("Financial Accounts");
                case "page-categories": return t("Categories");
                case "page-templates": return t("Transaction Templates");
                case "page-tags": return t("Manage Tags");
                case "page-tag-report": return document.getElementById("tagReportTitle")?.textContent || t("Spending by Tag");
                case "page-backup": return t("Export & Import");
                case "page-autolock": return t("Auto-Lock Settings");
                case "page-database": return t("Database");
                case "page-spending-breakdown": return t("Spending Breakdown");
                case "page-income-breakdown": return t("Income Breakdown");
                case "page-savings": return t("Savings Statement");
                case "page-networth-statement": return t("Net Worth Statement");
                case "page-portfolio-report": return t("Unit Trust Portfolio");
                case "page-owner-networth-report": return t("Financial Assets vs Real Estate");
                case "page-currency-report": return t("Net Worth by Currency");
                case "page-datasecurity": return t("Settings");
                case "page-navupdate": return t("Daily NAV Update");
                case "page-members": return t("Manage Members");
                case "page-member": return document.getElementById("memberPageTitle")?.textContent || t("Member");
                default: return t("Ledger");
            }
        }
